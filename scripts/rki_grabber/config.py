...
def _bool(value: Any, name: str, default: bool) -> bool:
    ...
    if type(value) is not bool:
        raise ValueError(f"{name} muss ein boolescher TOML-Wert sein")
    return value
...
def load_source_config(path: Path = DEFAULT_CONFIG_PATH) -> SourceConfig:
    ...
    if allowed_hosts != ["edoc.rki.de"]:
        raise ValueError("source.allowed_hosts darf ausschließlich edoc.rki.de enthalten")
    ...
    return SourceConfig(
        ...
        user_agent=str(network.get("user_agent", "RKI-EpidBull-Research-Downloader/2.0")),
        delay_seconds=_float(network.get("delay_seconds"), "delay_seconds", 1.25),
        timeout_seconds=_float(network.get("timeout_seconds"), "timeout_seconds", 60.0),
        max_redirects=_int(network.get("max_redirects"), "max_redirects", 5),
        max_listing_pages=_int(network.get("max_listing_pages"), "max_listing_pages", 10_000),
        max_html_bytes=_int(limits.get("max_html_bytes"), "max_html_bytes", 4 * 1024 * 1024),
        max_pdf_bytes=_int(limits.get("max_pdf_bytes"), "max_pdf_bytes", 256 * 1024 * 1024),
        robots_max_bytes=_int(limits.get("robots_max_bytes"), "robots_max_bytes", 512 * 1024),
        respect_robots=_bool(network.get("respect_robots"), "respect_robots", True),
    )
