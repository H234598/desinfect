...
def build_parser() -> argparse.ArgumentParser:
    """Build the strict runtime-status CLI parser."""
    ...
    start.add_argument(
        "--run-mode",
        choices=("plan", "materialize", "apply"),
        default="plan",
    )
    start.add_argument(
        "--storage-backend",
        choices=("lfs", "release", "object"),
        default="lfs",
    )
    ...
    update.add_argument("--completed-phases")
    ...
    finish.add_argument(
        "--status",
        choices=("success", "no_op", "blocked", "failed", "recovered"),
        required=True,
    )
    ...
    restore = sub.add_parser(
        "restore",
        help="Migrate/validate one snapshot and write it atomically",
    )
...
def main(argv: list[str] | None = None) -> int:
    """Execute one runtime-status command and return a stable exit code."""
    ...
    try:
        if args.command == "start":
            ...
            if args.recovery_level or args.recovery_action:
                recovery = {
                    ...
                    "action": (
                        ...
                        or "Manuelle Prüfung erforderlich"
                    ),
            ...
            completed_phases = (
                None
                if args.completed_phases is None
                else csv_values(args.completed_phases)
            )
            payload = update_run(
                source,
            ...
            source = load_json(args.input)
            current_public = load_json(args.public_status)
            finalized = update_run(
                ...
                phase="complete",
                completed_phases=[
                    *source.get("completed_phases", []),
                    "complete",
                ],
                error=source.get("error"),
                recovery=source.get("recovery"),
            )
            # Compute and validate both documents before persisting either one.
