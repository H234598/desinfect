...
3. Workflow installiert OpenVPN bei Bedarf erst nach Deploy und prüft danach ausschließlich den öffentlichen `GET /healthz` über einen temporären Tunnel: DE, dann NL, dann CH. Jede Verbindung braucht PID, TUN und VPN-Route; PID, TUN, Log sowie temporäre Secret-Dateien werden bei jedem Fallback und bei jedem Exit entfernt. `service`, `status` und `version` müssen exakt Workername, `ok` und aktuellem Git-SHA entsprechen.
