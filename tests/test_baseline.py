...
class BaselineTests(unittest.TestCase):
    ...
    def test_locked_decisions(self) -> None:
        data = json.loads((ROOT / "config/architecture-decisions.json").read_text(encoding="utf-8"))
    ...
    def test_requirement_counts(self) -> None:
        data = json.loads((ROOT / "docs/requirements/requirement-index.json").read_text(encoding="utf-8"))
        self.assertEqual(len(data["must_ids"]), 40)
        self.assertEqual(len(data["v2_ids"]), 169)
